...
# Modified by the authors, see the original: https://github.com/google/syzkaller/blob/master/tools/create-image.sh
...
PREINSTALL_PKGS=libpam-systemd,openssh-server,curl,tar,gcc,libc6-dev,time,strace,sudo,less,psmisc,selinux-utils,policycoreutils,checkpolicy,selinux-policy-default,firmware-atheros,debian-ports-archive-keyring,iw,hostapd,vim,bluez,bluez-tools,network-manager
...
display_help() {
    ...
    echo "   -s, --seek                 Image size (MB), default 4096 (4G)"
...
# Systemd log to kernel messages
sudo sed -i 's/#ForwardToKMsg=no/ForwardToKMsg=yes/' $DIR/etc/systemd/journald.conf
sudo sed -i 's/#Storage=auto/Storage=none/' $DIR/etc/systemd/journald.conf

# Disable NetworkManager, GDM & hostapd
sudo rm $DIR/etc/systemd/system/dbus-org.freedesktop.nm-dispatcher.service
sudo rm $DIR/etc/systemd/system/multi-user.target.wants/NetworkManager.service
sudo chroot $DIR /bin/bash -c "/lib/systemd/systemd-sysv-install disable hostapd"

# Add hostapd configuration
sudo cp hostapd.conf $DIR/etc/hostapd/hostapd.conf
sudo echo "DAEMON_CONF=\"/etc/hostapd/hostapd.conf\"" | sudo tee -a $DIR/etc/default/hostapd

# Add BT Scan
sudo cp bt-enable-connect.service $DIR/etc/systemd/system/bt-enable-connect.service

# Add permanent scan + ibss service
sudo cp permanent-scan.service $DIR/etc/systemd/system/permanent-scan.service
sudo cp ibss.service $DIR/etc/systemd/system/ibss.service

# Add Syzkaller Setup
sudo cp setup-syzkaller.service $DIR/etc/systemd/system/setup-syzkaller.service
sudo cp setup-syzkaller-hwsim.sh $DIR/root/setup-syzkaller-hwsim.sh
sudo chmod +x $DIR/root/setup-syzkaller-hwsim.sh

# Add permissive Bluetooth settings
sudo cp bluetooth.conf $DIR/etc/bluetooth/main.conf
